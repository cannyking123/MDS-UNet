...
def predict(model, config, logger):
    ...
    # * load datasetBs
    from dataloader2 import DatasetPredict as Dataset

    dataset = Dataset(config).subjects  # ! notice in predict.py should use Dataset(conf).subjects
    znorm = ZNormalization()

    name, pre_ls, rec_ls, jaccard_ls, dice_ls, hs95_ls = [], [], [], [], [],[]
    ...
    for i, item in enumerate(dataset):
        ...
        item = znorm(item)
        grid_sampler = tio.inference.GridSampler(item, patch_size=(config.patch_size), patch_overlap=(2, 2, 4))
        ...
        patch_loader = torch.utils.data.DataLoader(
            grid_sampler, batch_size=config.batch_size, shuffle=False, num_workers=1, pin_memory=False,persistent_workers=False
        )
        ...
        with torch.no_grad():
            for j, batch in enumerate(patch_loader):
                ...
                gt = batch["gt"]["data"].to(accelerator.device)
            ...
            input_path = item['source']['path']
            base_filename = os.path.basename(input_path)  # eg. 'volume001.nii.gz'
            base_filename = os.path.splitext(base_filename)[0]
            if base_filename.endswith('.nii'):
                base_filename = os.path.splitext(base_filename)[0]
            save_nii(pred_t, affine, base_filename, config)

            # * calculate metrics
            # precision, recall, jaccard, dice, hs95 = metric(gt_t, pred_t, spacing)
            precision, recall, jaccard, dice, hs95 = multiclass_metrics(gt_t, pred_t, num_classes=config.out_classes,spacing=spacing)
        ...
        del grid_sampler, patch_loader, pred_aggregator, gt_aggregator

    save_csv(name, pre_ls, rec_ls, jaccard_ls, dice_ls, hs95_ls, config)
...
def save_csv_ini(name, pre_ls, rec_ls, jaccard_ls, dice_ls, hs95_ls, config):
    import pandas as pd
    """
    陈老师原来保存csv的代码
    """
    ...
    data = {"name": name, "precision": pre_ls, "recall": rec_ls, "jaccard": jaccard_ls, "dice": dice_ls, "hs95": hs95_ls}
...
def save_csv(name, pre_ls, rec_ls, jaccard_ls, dice_ls, hs95_ls, config):
...
def save_nii(pred, affine, index, config):
    ...
    os.makedirs(save_base, exist_ok=True)
    pred_data = tio.ScalarImage(tensor=pred, affine=affine)
    pred_data.save(os.path.join(save_base, f"pred-{index}.nii.gz"))
...
def main(config):
    ...
    elif config.network == "HiPaS":
        from models.three_d.HiPaS import HiPaS_STS_Single # * 3d unet
        print("HiPaS")
        model = HiPaS_STS_Single(in_channels=config.in_classes, out_channels=config.out_classes)
